...
class FLAME(nn.Module):
    ...
    def __init__(self, template_dir):
        ...
        flame_model_path = os.path.join(template_dir, "flame2023_np.pkl")
        ...
        shapedirs = torch.cat([shapedirs[:, :, :self.n_shape], shapedirs[:, :, 300:300 + self.n_exp]], 2)
        self.register_buffer('shapedirs', shapedirs)
        # The pose components
        posedirs = to_np(flame_model.posedirs) # 5023 3 9*4
        num_pose_basis = posedirs.shape[-1] # 36 (9*4 neck jaw eyeballs)
    ...
    def forward(self, params):
        ...
        expr_params = params[1]
        pose_params = params[2]
        scale = params[3].reshape(-1, 1, 1)
        trans = params[4].reshape(-1, 1, 3)
        ...
        # global pose 3 + neck pose 3(000) + jaw pose 3(000), eye pose 6(000000) = 15
        template_vertices = self.v_template.unsqueeze(0).expand(batch_size, -1, -1)

        vertices, _, A = lbs(betas, full_pose, template_vertices,
        ...
        vertices = scale * (vertices + trans)
        landmarks3d = scale* (landmarks3d + trans)

        return vertices, landmarks3d, A

    def init_zero_params(self, batch_size=1, pose_params=None, scale_params=None, trans_params=None):
        shape_params = torch.zeros([batch_size, self.n_shape], dtype=torch.float32).cuda()
        expr_params = torch.zeros([batch_size, self.n_exp], dtype=torch.float32).cuda()

        if pose_params is None:
            pose_params = torch.tensor([[0, 0, 0, 0, 0, 0]], dtype=torch.float32).cuda().reshape(1, -1).repeat(batch_size, 1)
        else:
            if pose_params.size(1) != 6:
                pose_params = torch.cat([pose_params, torch.zeros_like(pose_params)], dim=1).cuda()

        if scale_params is None:
            scale = torch.ones([batch_size], dtype=torch.float32).cuda().reshape(batch_size, 1)*10
        else:
            scale = scale_params

        if trans_params is None:
            trans = torch.tensor([[0, 0, 0]], dtype=torch.float32).cuda().reshape(1, -1).repeat(batch_size, 1)
        else:
            trans = trans_params
        # trans = torch.tensor([[0, 0.1, 1.0]], dtype=torch.float32).cuda().reshape(1, -1).repeat(batch_size, 1)

        opt_params = nn.ParameterList([shape_params, expr_params, pose_params, scale, trans])
        return opt_params # shape expr pose scale trans
